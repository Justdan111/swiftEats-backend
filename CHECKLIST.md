...
- ✅ 6 migration files (users, restaurants/menu, orders/payments/cart)
- ✅ Database schema with proper FK relationships, indexes, constraints
- ✅ Users table with authentication support
- ✅ Go project structure initialized
- ✅ Dependencies: gorilla/mux, JWT, bcrypt, pgx, sqlc
- ✅ User authentication (register/login with JWT)
- ✅ Auth middleware for protected routes
- ✅ SQLC installed and configured
- ✅ SQLC generated all typed Go models and query functions
...
cmd/server/main.go
  ...
  └── Routes
      ...
      ├── GET /api/me (User service - protected)
      └── GET /health
...
# SwiftEats Backend — Checklist & Endpoints
  │   └── routes.go (route registration)
  │
  ├── db/ (GENERATED BY SQLC)
  │   ├── models.go (Restaurant, MenuItem, Order, OrderItem, Cart, Payment)
  │   ├── queries.sql.go (all SQLC query functions)
  ...
  │
  └── store/ (pgxpool-based connection pool)
```
...
- `internal/db/queries/queries.sql` → Add missing queries
**Effort:** 30 minutes
...
**Files to modify:**
- `cmd/server/main.go` → Switch to pgxpool + SQLC
- `internal/user/repository.go` → Use SQLC queries
- `internal/user/service.go` → Update for SQLC
- `internal/store/db.go` → Add initialization logic
...
- `internal/restaurant/repository.go` → Uses SQLC queries
- `internal/restaurant/service.go` → Business logic
...
POST   /api/cart/checkout         → Clear cart (for order)
```
...
GET    /api/orders/{id}         → Get order + items details
...
- If order inserts but items fail → broken order in DB
- If items insert but order fails → orphaned order_items
...
**Routes needed:**
```
POST /api/payments/init              → Initialize payment (create session)
POST /api/webhooks/payment           → Handle payment provider callback
...
5. Add items to cart
6. View cart
7. Create order → should return order_id
8. Check pgAdmin: verify rows in orders + order_items
9. Create order with same idempotency_key → should return same order (no duplicate)
...
|-------|---------|--------|--------|
| 1 | Database Setup | 2 hours | ✅ DONE |
| 2A | SQLC Queries | 0.5 hours | ⏳ NEXT |
| 2B | DB Connection Refactor | 0.75 hours | ⏳ NEXT |
| 3 | Restaurant | 1 hour | ⏳ NEXT |
...
### Current Issues:
1. **User module uses raw SQL** instead of SQLC → needs refactor
2. **No request validation** → will accept bad data
3. **No error codes** → hard to debug API issues
4. **No logging** → can't trace problems in production
5. **No transaction handling** in order creation → data consistency risk
6. **No payment provider integration** → need to pick one (Stripe/PayPal)
